...
import { saveToken, saveUser } from '../Account/userStorageService'
...
const Login = () => {
  ...
  const message = router.state?.message || ''
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (token && user) {
          ...
          if (user.role === 'ADMIN') {
            // router.push('/') // Navigate to /admin/DashboardCards
  ...
  return (
    <div className='login-container mt-8' dir={direction}>
      {message && (
        <p className='alert alert-warning'>
          {t('login.alert_warning')}: {t('login_redirect_message')}
        </p>
      )}
